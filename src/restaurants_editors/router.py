...
@router_restaurants_editors.post("/auth/restaurants-editors/sign_up", status_code=status.HTTP_201_CREATED)
...
@router_restaurants_editors.post("/auth/restaurants-editors/sign_in",
...
async def sign_in(
    ...
    restaurant_editor = await db.scalar(
        ...
        .where(User.roles.has(title="RESTAURANT_EDITOR"), User.username == username)
    )
...
@router_restaurants_editors.post("/auth/restaurants-editors/token/refresh", status_code=status.HTTP_201_CREATED)
...
@router_restaurants_editors.get("/restaurant-editors/", response_model=List[RestaurantEditorSchemas],
                                status_code=status.HTTP_200_OK)
async def get_restaurant_editors(
    ...
    stmt = await db.scalars(
        select(User)
        .options(selectinload(User.roles))
        .where(User.roles.has(title="RESTAURANT_EDITOR"))
...
@router_restaurants_editors.get("/restaurants-editors/{user_uuid}",
...
async def get_restaurant_editor_by_uuid(user_uuid: UUID,
    ...
    restaurant_editor = await db.scalar(
        select(User)
        .options(selectinload(User.roles))
        .where(User.roles.has(title="RESTAURANT_EDITOR"), User.uuid == user_uuid)
    )
...
@router_restaurants_editors.put("/restaurants-editors/{user_uuid}",
                                response_model=RestaurantEditorSchemas,
                                status_code=status.HTTP_200_OK)
...
@router_restaurants_editors.delete("/restaurants-editors/{user_uuid}",
